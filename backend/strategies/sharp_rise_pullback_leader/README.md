...
成交数量沿用当前策略执行配置：每笔目标金额为当时总资产的 `33%`，按 100 股整数手向下取整。仓位配置不参与信号筛选。
...
价格从已确认的持仓最高价回撤达到 4% 时卖出。优先使用 5 分钟线按时间顺序推进持仓最高价和止盈价，避免用日线 high/low 猜测有利的盘中顺序；若缺少 5 分钟线，只允许当日价格触发进入当日前已经确认的止盈价，当日新高留到下一交易日生效。

正常覆盖止盈价时按 `trailing_stop` 卖出；若开盘或后续 5 分钟 K 线直接跳空到止盈价下方，则按该时点开盘价卖出，避免以无法成交的更高价格进行乐观撮合。

策略不设置回撤止盈的最低浮盈门槛，也不保留其他卖出规则。
...
5. 动态回撤只使用买入后已经出现的价格；5 分钟线按时间升序处理，日线回退逻辑不使用当日未知先后顺序构造止盈成交。
