...
当前 `small_float_value` 策略已回到基准状态，不启用指数 MA20、Top500 牛熊指标、金叉死叉或其他市场环境过滤：

- 每个交易日 T 收盘后使用截至 T 日的宽表横截面计算目标池，并刷新观望池。
- 基础约束为普通主板非 ST、上市满 250 日、EPS 非负、价格和流通市值有效。
- 在基础池中使用不复权收盘价 `close` 筛选全市场最低价 10% 股票，再按 `circ_mv` 从小到大排序，选取第 1-10 名作为目标池。
- 买入和周频调仓只在自然周一开盘执行，使用周一之前已经写入观望池的目标池。
- 日常卖出规则仍每天执行：上一交易日涨停、T 日未继续涨停时按 T 日收盘价卖出。
