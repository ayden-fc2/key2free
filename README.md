...
当前 `small_float_value` 策略不再用单一指数 MA20 做市场环境过滤，而是在信号阶段用截至 T 日的小市值近似股票池合成牛熊指标：

- 策略只在交易周最后一个开市日收盘后计算并发出信号，供下一交易周第一个开市日开盘调仓使用；其他交易日 `select_signals` 直接返回空列表。交易周边界来自 `tushare.trade_cal`，节假日导致周五或周一休市时会自然顺延/提前。
- 每个交易日先按策略基础约束过滤普通主板非 ST、上市满 250 日、EPS 非负、价格和流通市值有效的股票。
- 在过滤后的股票中按 `circ_mv` 取最小的 500 只，样本不足 300 只时视为数据不足并空仓。
- 每日牛熊指标 = top500 平均换手率 + top500 平均收益率的 20 日波动率。
- 对牛熊指标计算 10 日短均线和 40 日长均线；短均线在长均线之上视为处于最近一次金叉之后，正常出信号；短均线在长均线之下视为处于最近一次死叉之后，`select_signals` 直接返回空列表。

该过滤是信号阶段的市场环境开关，不是交易阶段的临时判断。由于 T 日收盘后的信号会在下一次买入/调仓前进入观望池，环境差时空信号会使目标池为空，回测中的交易周首个开市日调仓逻辑会自然卖出不在目标池内的持仓。整个流程只使用截至 T 日的宽表历史数据，不读取 T+1 或更晚的股票或指数表现；交易日历属于事前可知信息，可用于判断交易周边界。
